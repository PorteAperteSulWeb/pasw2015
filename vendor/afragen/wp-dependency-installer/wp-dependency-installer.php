				...
				if ( $this->is_installed( $slug ) ) {
					if ( $is_optional ) {
						$this->notices[] = array(
							...
							'text'   => sprintf( esc_html__( 'Per un corretto funzionamento di PASW 2015 il plugin %s deve essere attivo.' ), $dependency['name'] ),
					...
					$this->notices[] = array(
						...
						'slug'   => $slug,
						/* translators: %s: Plugin name */
						'text'   => sprintf( esc_html__( 'Per un corretto funzionamento di PASW 2015 il plugin %s deve essere installato.' ), $dependency['name'] ),
						'source' => $dependency['source'],
					);
				} else {
